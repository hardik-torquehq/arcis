...
// Setup initializes a new Arcis. A Nop logger is set in Arcis.
func Setup(
	isCheckTx bool,
	feemarketGenesis *feemarkettypes.GenesisState,
) *Arcis {
	db := dbm.NewMemDB()
	app := NewArcis(log.NewNopLogger(), db, nil, true, map[int64]bool{}, DefaultNodeHome, 5, encoding.MakeConfig(ModuleBasics), simapp.EmptyAppOptions{})
	if !isCheckTx {
...
func SetupTestingApp() (ibctesting.TestingApp, map[string]json.RawMessage) {
	...
	app := NewArcis(log.NewNopLogger(), db, nil, true, map[int64]bool{}, DefaultNodeHome, 5, cfg, simapp.EmptyAppOptions{})
