	...
	testCases := []struct {
	...
	}{
		{
			...
			banktypes.Metadata{
				...
				Name:        "Arcis",
				Symbol:      "ARCIS",
				Description: "EVM, staking and governance denom of Arcis",
				DenomUnits: []*banktypes.DenomUnit{
					{
						Denom:    "aarcis",
						Exponent: 0,
						Aliases:  []string{"atto arcis"},
					},
					{
						Denom:    "arcis",
						Exponent: 18,
...
func TestEqualAliases(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			[]string{"atto arcis", "aarcis"},
			[]string{"aarcis", "atto arcis"},
			false,
		},
		{
			"same values, sorted",
