...
A user can query the Arcis `x/vesting` module using the CLI, gRPC, or REST.

## CLI

Find below a list of `arcisd` commands added with the `x/vesting` module. You can obtain the full list by using the `arcisd -h` command.
...
```go
arcisd query vesting balances [address] [flags]
```
...
```go
arcisd tx vesting create-clawback-vesting-account [to_address] [flags]
```
...
```go
arcisd tx vesting clawback [address] [flags]
```
...
| ------ | -------------------------------------- | -------------------------------------- |
| `gRPC` | `arcis.vesting.v1.Query/Balances`      | Gets locked, unvested and vested coins |
| `GET`  | `/arcis/vesting/v1/balances/{address}` | Gets locked, unvested and vested coins |

### Transactions

| Verb   | Method                                                 | Description                      |
| ------ | ------------------------------------------------------ | -------------------------------- |
| `gRPC` | `arcis.vesting.v1.Msg/CreateClawbackVestingAccount`    | Creates clawback vesting account |
| `gRPC` | `/arcis.vesting.v1.Msg/Clawback`                       | Performs clawback                |
| `GET`  | `/arcis/vesting/v1/tx/create_clawback_vesting_account` | Creates clawback vesting account |
| `GET`  | `/arcis/vesting/v1/tx/clawback`                        | Performs clawback                |
