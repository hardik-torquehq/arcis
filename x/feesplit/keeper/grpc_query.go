...
func (k Keeper) DeployerFeeSplits( // nolint: dupl
	...
	if err != nil {
		return nil, status.Errorf(
			codes.InvalidArgument,
			"invalid format for deployer %s, should be bech32 ('arcis...')", req.DeployerAddress,
		)
	}
...
func (k Keeper) WithdrawerFeeSplits( // nolint: dupl
	...
	if err != nil {
		return nil, status.Errorf(
			codes.InvalidArgument,
			"invalid format for withdraw addr %s, should be bech32 ('arcis...')", req.WithdrawerAddress,
		)
	}
