...
func (k Keeper) OnRecvPacket(
	...
	// readable prefix (HRP) of the sender to `arcis1`
	...
	// supported on Arcis. To prevent funds getting stuck, return an error, unless
	...
	if sameAddress && !fromEVMChain {
		...
		case senderRecordFound && !senderClaimsRecord.HasClaimedAny():
			...
			return channeltypes.NewErrorAcknowledgement(
				sdkerrors.Wrapf(
					arcis.ErrKeyTypeNotSupported, "receiver address %s is not a valid ethereum address", recipientBech32,
				).Error(),
			)
		default:
