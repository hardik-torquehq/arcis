...
//  - sends over all Arcis native tokens
...
func (k Keeper) OnRecvPacket(
	...
	// Get addresses in `arcis1` and the original bech32 format
	...
	// a sender's own account on Arcis (sender == recipient)
	...
	if account != nil && arcis.IsSupportedKey(account.GetPubKey()) {
		return ack
	}
	...
	k.bankKeeper.IterateAccountBalances(ctx, recipient, func(coin sdk.Coin) (stop bool) {
		...
		err = k.transferKeeper.SendTransfer(
			...
			recipient,                 // recipient is the address in the Arcis chain
...
// the IBC denomination, i.e port and channel on Arcis for the voucher. It
// returns an error if:
