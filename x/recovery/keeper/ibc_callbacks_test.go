...
func (suite *KeeperTestSuite) TestOnRecvPacket() {
	...
	// Setup Cosmos <=> Arcis IBC relayer
	denom := "uatom"
	sourceChannel := "channel-292"
	arcisChannel := claimstypes.DefaultAuthorizedChannels[1]
	path := fmt.Sprintf("%s/%s", transfertypes.PortID, arcisChannel)
	...
	coins := sdk.NewCoins(
		sdk.NewCoin("aarcis", sdk.NewInt(1000)),
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", ethsecpAddrArcis, ethsecpAddrCosmos)
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", "arcis", ethsecpAddrCosmos)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, arcisChannel, timeoutHeight, 0)
			},
			false,
			false,
			coins,
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", ethsecpAddrArcis, "badbadhf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, arcisChannel, timeoutHeight, 0)
			},
			false,
		...
		},
		{
			...
			func() {
				...
				otherSecpAddrArcis := sdk.AccAddress(pk1.PubKey().Address()).String()

				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, otherSecpAddrArcis)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, arcisChannel, timeoutHeight, 0)
			},
			true,
		...
		},
		{
			...
			func() {
				...
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", ethsecpAddrCosmos, ethsecpAddrArcis)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, arcisChannel, timeoutHeight, 0)
			},
			true,
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, secpAddrArcis)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, arcisChannel, timeoutHeight, 0)
		...
		{
			"recovery - send uatom from cosmos to arcis",
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, secpAddrArcis)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, arcisChannel, timeoutHeight, 0)
		...
		{
			"recovery - send ibc/uosmo from cosmos to arcis",
			func() {
				denom = ibcOsmoDenom
		...
		{
			"recovery - send uosmo from osmosis to arcis",
			func() {
				// Setup Osmosis <=> Arcis IBC relayer
				denom = "uosmo"
				sourceChannel = "channel-204"
				arcisChannel = claimstypes.DefaultAuthorizedChannels[0]
				path = fmt.Sprintf("%s/%s", transfertypes.PortID, arcisChannel)
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.app.IBCKeeper.ChannelKeeper.SetChannel(suite.ctx, transfertypes.PortID, arcisChannel, channel)

			// Set Next Sequence Send
			suite.app.IBCKeeper.ChannelKeeper.SetNextSequenceSend(suite.ctx, transfertypes.PortID, arcisChannel, 1)
			...
			// Fund receiver account with ARCIS, ERC20 coins and IBC vouchers
			testutil.FundAccount(suite.app.BankKeeper, suite.ctx, secpAddr, coins)
...
func (suite *KeeperTestSuite) TestOnRecvPacketFailTransfer() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, secpAddrArcis)
			packet := channeltypes.NewPacket(transfer.GetBytes(), 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, arcisChannel, timeoutHeight, 0)
			...
			// Fund receiver account with ARCIS
			coins := sdk.NewCoins(
				sdk.NewCoin("aarcis", sdk.NewInt(1000)),
				sdk.NewCoin(ibcAtomDenom, sdk.NewInt(1000)),
			)
