...
SIMAPP = ./app
HTTPS_GIT := https://github.com/Ambiplatforms-TORQUE/arcis.git
DOCKER := $(shell which docker)
DOCKER_BUF := $(DOCKER) run --rm -v $(CURDIR):/workspace --workdir /workspace bufbuild/buf
NAMESPACE := ambiplatforms-torquehq
PROJECT := arcis
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=arcis \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(ARCIS_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm arcis || true
	# create a new container from the latest image
	$(DOCKER) create --name arcis -t -i ${DOCKER_IMAGE}:latest arcis
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp arcis:/usr/bin/arcisd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/Ambiplatforms-TORQUE/arcis/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(ARCIS_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(ARCIS_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(ARCIS_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(ARCIS_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/Ambiplatforms-TORQUE/arcis
.PHONY: format
...
ETHERMINT_URL      	= https://raw.githubusercontent.com/Ambiplatforms-TORQUE/ethermint/v0.1.1
