...
## default port prefixes for arcisd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/arcisd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/arcisd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/arcisd add-genesis-account \
    "$("$PWD"/build/arcisd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000aarcis,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/arcisd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/arcisd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/arcisd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/arcisd start --pruning=nothing --rpc.unsafe \
    ...
    ARCIS_PID=$!
    echo "started arcis node, pid=$ARCIS_PID"
    # add PID to array
    arr+=("$ARCIS_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test arcis node $HOST_RPC ..."
...
stop_func() {
    ARCIS_PID=$i
    echo "shutting down node, pid=$ARCIS_PID ..."
