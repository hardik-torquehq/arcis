...
speed and transaction amount. For Arcis, we are talking about close to
...
I believe this was disabled by default on Arcis, but listing it in any
...
will risk database corruption if the `arcisd` is killed for any reason.
...
Below is the disk usage after two weeks of Arcis Arsia Mons testnet. The default
