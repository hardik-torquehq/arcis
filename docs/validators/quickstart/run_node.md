...
Configure and run an Arcis node {synopsis}

## Pre-requisite Readings

- [Installation](./installation.md) {prereq}
- [`arcisd`](./binary.md) {prereq}
...
```bash
arcisd start --json-rpc.enable=true --json-rpc.api="eth,web3,net"
```

## Key Management

To run a node with the same key every time: replace `arcisd keys add $KEY` in `./init.sh` with:

```bash
echo "your mnemonic here" | arcisd keys add $KEY --recover
```

::: tip
Arcis currently only supports 24 word mnemonics.
:::
...
To export your arcis key as an Ethereum private key (for use with [Metamask](./../../users/wallets/metamask.md) for example):

```bash
arcisd keys unsafe-export-eth-key $KEY
```
...
Arcis supports using a file or OS keyring backend for key storage. To create and use a file
stored key instead of defaulting to the OS keyring, add the flag `--keyring-backend file` to any
relevant command and the password prompt will occur through the command line. This can also be saved
as a CLI config option with:

```bash
arcisd config keyring-backend file
```
...
::: danger
If you are running a **validator node**, always be careful when doing `arcisd unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
:::
...
```shell
rate(arcisd_transactions_processed[1m])
```
...
Learn about running a Arcis [testnet](./../testnet.md) {hide}
