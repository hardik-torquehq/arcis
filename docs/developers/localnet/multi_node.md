...
This command creates a 4-node network using the `arcisdnode` Docker image.
The ports for each node are found in this table:

| Node ID          | P2P Port | Tendermint RPC Port | REST/ Ethereum JSON-RPC Port | WebSocket Port |
|------------------|----------|---------------------|------------------------------|----------------|
| `arcisnode0` | `26656`  | `26657`             | `8545`                       | `8546`         |
| `arcisnode1` | `26659`  | `26660`             | `8547`                       | `8548`         |
| `arcisnode2` | `26661`  | `26662`             | `8549`                       | `8550`         |
| `arcisnode3` | `26663`  | `26664`             | `8551`                       | `8552`         |
...
```bash
...
Creating network "arcis_localnet" with driver "bridge"
Creating arcisdnode0 ... done
Creating arcisdnode2 ... done
Creating arcisdnode1 ... done
Creating arcisdnode3 ... done
```
...
calling the `arcisd testnet` command. This outputs a handful of files in the
`./build` directory:
...
Each `./build/nodeN` directory is mounted to the `/arcisd` directory in each container.
...
```bash
# node 0: daemon logs
docker exec arcisdnode0 tail arcisd.log
...
To interact with `arcisd` and start querying state or creating txs, you use the
`arcisd` directory of any given node as your `home`, for example:

```bash
arcisd keys list --home ./build/node0/arcisd
```
...
::: tip
**Note**: Each node's seed is located at `./build/nodeN/arcisd/key_seed.json` and can be restored to the CLI using the `arcisd keys add --restore` command
:::
...
BINARY=arcis make localnet-start
```
