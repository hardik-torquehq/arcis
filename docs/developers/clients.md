...
The Arcis supports different clients in order to support Cosmos and Ethereum transactions
and queries:

|                                                        | Description                                                                          | Default Port |
| ------------------------------------------------------ | ------------------------------------------------------------------------------------ | ------------ |
| **Cosmos [gRPC](#cosmos-grpc)**                        | Query or send Arcis transactions using gRPC                                          | `9090`       |
| **Cosmos REST ([gRPC-Gateway](#cosmos-grpc-gateway))** | Query or send Arcis transactions using an HTTP RESTful API                           | `9091`       |
...
| **Command Line Interface ([CLI](#cli))**               | Query or send Arcis transactions using your Terminal or Console.                     | N/A          |

## Cosmos gRPC

Arcis exposes gRPC endpoints for all the integrated Cosmos SDK modules. This makes it easier for
...
See the list of supported gRPC-Gateway API endpoints for the Arcis testnet [here](https://api.arcis.dev/).

## Ethereum JSON-RPC

<!-- TODO: Link JSON-RPC docs -->

Arcis supports most of the standard [JSON-RPC APIs](./json-rpc/server.md) to connect with existing Ethereum-compatible web3 tooling.
...
Check the `Events` page to obtain the event list of each supported module on Arcis.
:::
...
Users can use the `{{ $themeConfig.project.binary }}` binary to interact directly with an Arcis node
though the CLI.
