...
Learn how to deploy a simple Solidity-based smart contract to Arcis using the Truffle environment {synopsis}
...
This will allow your contract to connect to your Arcis local node.
...
You should see incoming deployment logs in the Arcis daemon Terminal tab for each transaction (one to deploy `Migrations.sol` and the other to deploy `Counter.sol`).
...
Now, you can run the Truffle tests using the Arcis node using the `test` command:
