...
Arcis supports all standard web3 JSON-RPC APIs. You can find documentation for these APIs on the [`JSON-RPC Methods`](./endpoints.md) page.

JSON-RPC is provided on multiple transports. Arcis supports JSON-RPC over HTTP and WebSocket. Transports must be enabled through command-line flags or through the `app.toml` configuration file. For more details see the []
...
When requests are made that act on the state of Arcis, the last default block parameter determines the height of the block.
