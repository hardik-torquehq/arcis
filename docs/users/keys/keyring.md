...
```bash
arcisd keys add mykey

# Put the generated address in a variable for later use.
MY_VALIDATOR_ADDRESS=$(arcisd keys show mykey -a)
```
...
::: warning
**NOTE**: Cosmos `secp256k1` keys are not supported on Arcis due to compatibility issues with Ethereum transactions.
:::
...
yes $KEYPASSWD | arcisd keys add me
yes $KEYPASSWD | arcisd keys show me
# start arcisd with keyring-backend flag
arcisd --keyring-backend=file start
```
