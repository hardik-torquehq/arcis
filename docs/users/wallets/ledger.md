...
Get started with your Ledger hardware wallet on Arcis {synopsis}
...
If you want to connect to Arcis mainnet and Arcis testnet, you can use the Ethereum Ledger app on Ledger Live by setting the chain ID.
...
::: tip
Follow our [Metamask Guide](./metamask.md) to add the Arcis Mainnet and Testnet to your Settings
:::
...
### Step-by Cosmos transaction using Arcis.me

1. **Get your address in both encodings**

After connecting the Ledger wallet to Metamask and connecting to the [https://arcis.me](https://arcis.me) webpage, it will display our wallet formatted on `bech32` and `hex` representation, we need these values to make sure that the message that we are going to sign is the correct one.
...
- `msgs`: This is the content of the cosmos transaction, in this example, we need to make sure that we are using a MsgSend, and that the *to_address* is the one that we want to send the founds. Also, we can verify that we are actually sending *10000aarcis* to that wallet.
...
This step should be done automatically by the same service that generated the message, in this case, [arcis.me](http://arcis.me) will broadcast the transaction for you.

![txsent.png](./../../img/txsent.png)

### Common errors

- Make sure that the Ethereum Ledger app is installed. The Cosmos Ledger app is not supported on the Arcis chain at the moment (see [FAQ](#faq)).
...
- If you have Metamask correctly configured, the balance on the extension will be displayed as `ARCIS`, but on the Ledger device it will be displayed as `ETH`.
...
Cosmos `secp256k1` keys are not supported on Arcis with Ledger. Only Ethereum keys (`eth_secp256k1`) can be generated with Ledger.

2. **I can’t generate keys using the CLI with `arcisd` with the `--ledger` flag**

CLI bindings with `arcisd` binary are not currently supported. In the meantime, you can use the Ethereum Ledger App with EIP712 using [arcis.me](https://arcis.me). See the [`EIP712 Signing`](#eip712-signing) section for reference.

3. **I can’t generate a key for the Arcis native multisig using the `arcisd` CLI and and Ledger**

You can generate a multisig wallet using the `arcisd` CLI, although the `--ledger` option is not available at the moment.

4. **I can’t use Metamask or Keplr with the Cosmos Ledger app**

Since Arcis only support Ethereum keys and uses the same HD path as Ethereum, the Cosmos Ledger app doesn’t work to sign cosmos transactions.
